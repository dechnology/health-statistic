...
func (uq *UserQuery) QueryHealthkit() *HealthKitQuery {
	query := (&HealthKitClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryDeegoo chains the current query on the "deegoo" edge.
func (uq *UserQuery) QueryDeegoo() *DeegooQuery {
	query := (&DeegooClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withHealthkit:              uq.withHealthkit.Clone(),
		withDeegoo:                 uq.withDeegoo.Clone(),
...
func (uq *UserQuery) WithHealthkit(opts ...func(*HealthKitQuery)) *UserQuery {
	query := (&HealthKitClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withHealthkit = query
	return uq
}

// WithDeegoo tells the query-builder to eager-load the nodes that are connected to
// the "deegoo" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithDeegoo(opts ...func(*DeegooQuery)) *UserQuery {
	query := (&DeegooClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withHealthkit; query != nil {
		if err := uq.loadHealthkit(ctx, query, nodes,
			func(n *User) { n.Edges.Healthkit = []*HealthKit{} },
			func(n *User, e *HealthKit) { n.Edges.Healthkit = append(n.Edges.Healthkit, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withDeegoo; query != nil {
		if err := uq.loadDeegoo(ctx, query, nodes,
			func(n *User) { n.Edges.Deegoo = []*Deegoo{} },
			func(n *User, e *Deegoo) { n.Edges.Deegoo = append(n.Edges.Deegoo, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadQuestionnaireResponses(ctx context.Context, query *QuestionnaireResponseQuery, nodes []*User, init func(*User), assign func(*User, *QuestionnaireResponse)) error {
	...
	query.withFKs = true
	query.Where(predicate.QuestionnaireResponse(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.QuestionnaireResponsesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_questionnaire_responses
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_questionnaire_responses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadHealthkit(ctx context.Context, query *HealthKitQuery, nodes []*User, init func(*User), assign func(*User, *HealthKit)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.HealthKit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.HealthkitColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_healthkit
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_healthkit" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_healthkit" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadDeegoo(ctx context.Context, query *DeegooQuery, nodes []*User, init func(*User), assign func(*User, *Deegoo)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Deegoo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DeegooColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_deegoo
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_deegoo" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_deegoo" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
