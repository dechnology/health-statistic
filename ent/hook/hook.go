...
// Mutate calls f(ctx, m).
func (f HealthKitFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HealthKitMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HealthKitMutation", m)
}
...
// Mutate calls f(ctx, m).
func (f QuestionnaireFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.QuestionnaireMutation); ok {
		return f(ctx, mv)
	}
