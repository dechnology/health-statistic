...
func (qq *QuestionnaireQuery) QueryQuestions() *QuestionQuery {
...
func (qq *QuestionnaireQuery) WithQuestions(opts ...func(*QuestionQuery)) *QuestionnaireQuery {
	query := (&QuestionClient{config: qq.config}).Query()
...
//		GroupBy(questionnaire.FieldName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Questionnaire.Query().
//		Select(questionnaire.FieldName).
//		Scan(ctx, &v)
...
func (qq *QuestionnaireQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Questionnaire, error) {
	...
	if query := qq.withQuestions; query != nil {
		if err := qq.loadQuestions(ctx, query, nodes,
...
func (qq *QuestionnaireQuery) loadQuestionnaireResponses(ctx context.Context, query *QuestionnaireResponseQuery, nodes []*Questionnaire, init func(*Questionnaire), assign func(*Questionnaire, *QuestionnaireResponse)) error {
	...
	query.Where(predicate.QuestionnaireResponse(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(questionnaire.QuestionnaireResponsesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.questionnaire_questionnaire_responses
		if fk == nil {
			return fmt.Errorf(`foreign-key "questionnaire_questionnaire_responses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "questionnaire_questionnaire_responses" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
